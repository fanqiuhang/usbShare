...
//    }
    ...
    public static String freeAccess(String ip, int port, String id, int usbPort, int forceClose){
        return JSON.toJSONString(freeAccessToResult(ip,port,id,usbPort,forceClose));
    }
     ...
     */
    public static UsbshareResult freeAccessToResult(String id, int usbPort) {
        return freeAccessToResult(UsbshareConstants.DEFAULT_IP,UsbshareConstants.DEFAULT_PORT,id,usbPort);
    }
    /**
     * 释放终端usb设备使用权
     * @param ip ip地址
     ...
     * @return UsbshareResult
     */
    public static UsbshareResult freeAccessToResult(String ip, int port, String id, int usbPort) {
        return freeAccessToResult(ip,port,id,usbPort,0);
    }
    /**
     * 释放终端usb设备使用权
     * @param ip ip地址
     ...
     * @return UsbshareResult
     */
    public static UsbshareResult freeAccessToResult(String ip, int port, String id, int usbPort, int forceClose) {
        //检查合法性
        if (!UsbshareSendSocketUtil.checkId(id)) {
            return new UsbshareResult("端口格式错误");
        }
        ...
        if ("unuse".equals(state)) {
            return new UsbshareResult();
        } else if ((!"unuse".equals(state)) && (!"inuse".equals(state))){
            return new UsbshareResult(state);
        }
        //发起释放请求
        ...
        if (ss != null && list.length > 4 && "-95".equals(list[4])) {
            ...
            return new UsbshareResult(UsbshareUtils.analyseError(list));
        }
    }
...
//    }
    ...
    public static String getAccess(String ip,int port,String id ,int usbPort){
        return JSON.toJSONString(getAccessToResult(ip,port,id,usbPort));
    }
     ...
     * @return UsbshareResult
     */
    public static UsbshareResult getAccessToResult(String id ,int usbPort) {
        return getAccessToResult(UsbshareConstants.DEFAULT_IP,UsbshareConstants.DEFAULT_PORT,id,usbPort);
    }
    /**
     * 获取终端usb设备使用权
     * @param ip IP地址
     ...
     * @return UsbshareResult
     */
    public static UsbshareResult getAccessToResult(String ip,int port,String id ,int usbPort) {
        //检查合法性
        if (!UsbshareSendSocketUtil.checkId(id)) {
            return new UsbshareResult("端口格式错误");
        }
    ...
    public static String queryAllPortDetails(String ip, int port) {
        return JSON.toJSONString(queryAllPortDetailsToNative(ip,port));
    }
    /**
     * 默认，本机，端口8110
     * @return
     */
    public static UsbshareNative queryAllPortDetailsToNative() {
        return Usbshare.queryAllPortDetailsToNative(UsbshareConstants.DEFAULT_IP,UsbshareConstants.DEFAULT_PORT);
    }
    /**
     * 查询本机已打开的全部端口
     * @param ip
     ...
     * @return
     */
    public static UsbshareNative queryAllPortDetailsToNative(String ip, int port) {
        ...
        if (ss != null && list.length > 4 && "-89".equals(list[4])) {
            ...
            return new UsbshareNative(UsbshareUtils.analyseError(list));
        }
    }
...
//    }
    ...
    public static String queryPortDetail(String ip, int port, String id , int usbPort){
        return JSON.toJSONString(queryPortDetailToPortDetail(ip, port, id, usbPort));
    }
    /**
     * 获取终端usb端口详细信息
     * @param id 终端ID
     ...
     */
    public static UsbsharePortDetail queryPortDetailToPortDetail(String id , int usbPort) {
        return queryPortDetailToPortDetail(UsbshareConstants.DEFAULT_IP,UsbshareConstants.DEFAULT_PORT,id , usbPort);
    }
    /**
     * 获取终端usb端口详细信息
     * @param ip id地址
     ...
     * @return UsbsharePortDetail
     */
    public static UsbsharePortDetail queryPortDetailToPortDetail(String ip, int port, String id , int usbPort) {
        //检查合法性
        if (!UsbshareSendSocketUtil.checkId(id)) {
            return new UsbsharePortDetail("端口格式错误");
        }
        //发起请求
        ...
        if (ss != null && list.length > 4 && "-94".equals(list[4])) {
            ...
            return new UsbsharePortDetail(UsbshareUtils.analyseError(list));
        }
    }
}
